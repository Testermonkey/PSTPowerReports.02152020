...
namespace PSTPowerReports
{
    class PSTPowerLogs
    {
            ...
            try
            {
                ...
                else
                {
                    foreach (string cmd in args)
                    {
                        ...
                        else if (string.Compare(cmd.ToLower(), 0, "/active=", 0, 8) == 0)
                        {
                            string[] cmds = cmd.Split('=');
                            if (cmds.Count() > 1)
                            {
                                if (cmds[1].ToLower().Equals("true"))
                                {
                                    reportData.ShowActiveEnergy = true;
                                    reportinfo += cmds[1];
                                }
                            }
                        }
        ...
        private bool GetPowerData(string pstDeviceFolder, out PowerData powerData)
        {
            ...
            if (PSTDataProcessing.GetDrips(pstDeviceFolder, out double HwDrips, out double SwDrips, out double aedr, reportData.ShowActiveEnergy))
            {
            ...
            if (PSTDataProcessing.GetPowerSliderTypeFromDevice(pstDeviceFolder, out bool ACFlag, out PowerSliderType pSliderFromDevice, out double bateryTotal, 
        ...
        {
            Console.WriteLine("     PSTPowerReports [/h1=\"Header1\"] [/h2=\"Header2\"] [/PSTVer=\"2.3.1\"] /dir=\"c:\\TestResultFolders\"  [/active=true]");
            ...
            Console.WriteLine("         /active=true     - default is false to display all Zeros in the ActiveEnergyDrain column, true will fill report with ActiveEnergyDrain info.");
        }
    }
}
