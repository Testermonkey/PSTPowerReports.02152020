...
namespace PSTPowerReports
{
    class PSTDataProcessing
    {
        ...
        static public bool GetPowerSliderTypeFromDevice(string deviceFolder, out bool isAC, out PowerSliderType pwrMode, out double totalBatLevel, out double bat1Level, out double bat2Level)
        {
            pwrMode = PowerSliderType.None;
            bat1Level = 0.0;
            bat2Level = 0.0;
                ...
                if (File.Exists(powerSliderStatusFile))
                {
                    ...
                    if (psStatusFileList.Count() > 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(powerSliderStatusRecord))
                        {
                            ...
                            {
                                if (pssColumnToken[ii].Trim().ToLower().Equals("power") || pssColumnToken[ii].Trim().ToLower().Equals("powertype"))
                                {
        ...
        static public bool CheckMemoryDump(string deviceFolder, out bool hasMemDump, out List<string> bugcheckDetails, out List<string> memDumpList)
        {
            ...
            string crashReportPath = deviceFolder + @"\BSOD";

            // May not need this
            //string crashReportFile = deviceFolder + @"\BSOD" + @"\Analysis_*.txt";
            string memDumpDir = deviceFolder + @"\MemDump";

            // Returns true if file CrashReport.csv contains "CRASHED,"
            // This will change to detect the \BSOD\Analysis*.txt
            hasMemDump = false;

            // bugcheckDetails returns list of Dumps
            bugcheckDetails = new List<string>();

            // memDumpList returns list of dumps with paths
            memDumpList = new List<string>();

            try
            {
                // First check if any \BSOD\
                if (!Directory.Exists(crashReportPath))
                {
                    return false;
                }
                hasMemDump = true;
                foreach (string file in Directory.GetFiles(crashReportPath))
                {
                    ...
                    using (StreamReader sr = new StreamReader(file))
                    {
                        ...
                        while ((line = sr.ReadLine()) != null)
                        {
                            ...
                            if (line.Contains("FAILURE_ID_HASH_STRING:"))
                            {
                                bugcheckDetails.Add(line);
                            }

                            if (line.Contains("FAILURE_ID_HASH:"))
                            {
                                bugcheckDetails.Add(line);
                            }
                        }
                        ...
                        sr.Close();
                        Console.WriteLine(bugcheckDetails[1]);
                     }
                }
                // This section for collecting \MemDump\,Dumps will remain unchanged
        ...
        static public bool GetDrips(string deviceFolder, out double HwDripRate, out double SwDripRate, out double ActiveEnergyDrainRate)
        {
